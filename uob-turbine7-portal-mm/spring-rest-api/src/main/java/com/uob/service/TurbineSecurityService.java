...
public class TurbineSecurityService {
    ...
    public List<Group> getAllGroups() throws FulcrumSecurityException {
        try {
            ...
            // Safely convert GroupSet to List, ensuring proper casting
            for (Object obj : groupSet.getSet()) {
                if (obj instanceof Group) {
                    groups.add((Group) obj);
                } else {
                    log.warn("Object in GroupSet is not a Group instance: " + obj.getClass().getName());
                }
            }
            
            return groups;
        } catch (ClassCastException e) {
            log.error("ClassCastException getting all groups - GtpGroup may not properly implement Group interface", e);
            throw new FulcrumSecurityException("Error retrieving groups: " + e.getMessage(), e);
        } catch (Exception e) {
    ...
    public Group getGroupByName(String groupName) throws UnknownEntityException {
        try {
            ...
            // Ensure the returned group is properly cast
            if (group == null) {
                throw new UnknownEntityException("Group not found: " + groupName);
            }
            
            return group;
        } catch (ClassCastException e) {
            log.error("ClassCastException getting group: " + groupName + " - GtpGroup may not properly implement Group interface", e);

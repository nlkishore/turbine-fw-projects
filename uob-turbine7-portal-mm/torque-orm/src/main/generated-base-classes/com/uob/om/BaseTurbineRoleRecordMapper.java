...
public class BaseTurbineRoleRecordMapper implements RecordMapper<TurbineRole>
{
    /** Serial version */
    private static final long serialVersionUID = 1769223168931L;

    /** The class log. */
    private static Log log
            = LogFactory.getLog(BaseTurbineRoleRecordMapper.class);
    ...
    {
        this.strategy = new MappingStrategy<TurbineRole>();
    }
     ...
     *         than the columns in the TURBINE_ROLE table.
     */
    public TurbineRole processRow(
                ResultSet resultSet,
                int offset,
                Criteria criteria)
            throws TorqueException
    {
        ...
        try 
        {
            ...
            if (criteria == null)
            {
                turbineRole.setEntityId(
                        getEntityId(resultSet, offset + 1));
                turbineRole.setEntityName(
            ...
            else
            {
                ...
                for (Column column : columnsWithoutOffset)
                {
                            ...
                            column.getSqlExpression()))
                    {
                        ...
                        {
                            turbineRole.setEntityId( getEntityId(resultSet, totalOffset));
                        }
                            ...
                            column.getSqlExpression()))
                    {
                        ...
                        {
                            turbineRole.setEntityName( getEntityName(resultSet, totalOffset));
                        }
                ...
                if (useMappingStrategy)
                {
                  ...
                  this.strategy.execute(resultSet, turbineRole);
                }
            }
     ...
     * the field entityId in TurbineRole.
     *
     ...
     * Reads the value of the <code>columnIndex</code>'th column
     * in the <code>resultSet</code> so that it can be used to set
     * the field entityName in TurbineRole.
     *
     * @param resultSet the ResultSet to read from, not null.
     * @param columnIndex the index in the resultSet which should be read.
     *

...
public class BaseGtpPermissionRecordMapper implements RecordMapper<GtpPermission>
{
    /** Serial version */
    private static final long serialVersionUID = 1769223168530L;

    /** The class log. */
    private static Log log
            = LogFactory.getLog(BaseGtpPermissionRecordMapper.class);
            
    private static final String PERMISSION_ID_EXPRESSION = BaseGtpPermissionPeer.PERMISSION_ID.getSqlExpression();
    private static final String PERMISSION_NAME_EXPRESSION = BaseGtpPermissionPeer.PERMISSION_NAME.getSqlExpression();

    private boolean useMappingStrategy = true;

    private MappingStrategy<GtpPermission> strategy;
    
    public void initStrategy()
    {
        this.strategy = new MappingStrategy<GtpPermission>();
    }
     ...
     *         than the columns in the GTP_PERMISSION table.
     */
    public GtpPermission processRow(
                ResultSet resultSet,
                int offset,
                Criteria criteria)
            throws TorqueException
    {
        ...
        try 
        {
            gtpPermission.setLoading(true);
            if (criteria == null)
            {
                gtpPermission.setEntityId(
                        getEntityId(resultSet, offset + 1));
                gtpPermission.setEntityName(
            ...
            else
            {
                ...
                for (Column column : columnsWithoutOffset)
                {
                            ...
                            column.getSqlExpression()))
                    {
                        ...
                        {
                            gtpPermission.setEntityId( getEntityId(resultSet, totalOffset));
                        }
                            ...
                            column.getSqlExpression()))
                    {
                        ...
                        {
                            gtpPermission.setEntityName( getEntityName(resultSet, totalOffset));
                        }
                ...
                if (useMappingStrategy)
                {
                  ...
                  this.strategy.execute(resultSet, gtpPermission);
                }
            }
            gtpPermission.setNew(false);
            gtpPermission.setModified(false);
        }
        finally
        {
            gtpPermission.setLoading(false);
        }
        return gtpPermission;
    }

    /**
     * Reads the value of the <code>columnIndex</code>'th column
     * in the <code>resultSet</code> so that it can be used to set
     * the field entityId in GtpPermission.
     *
     ...
     * Reads the value of the <code>columnIndex</code>'th column
     * in the <code>resultSet</code> so that it can be used to set
     * the field entityName in GtpPermission.
     *
     * @param resultSet the ResultSet to read from, not null.
     * @param columnIndex the index in the resultSet which should be read.
     *

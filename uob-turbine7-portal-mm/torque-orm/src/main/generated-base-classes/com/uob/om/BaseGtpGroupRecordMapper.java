...
public class BaseGtpGroupRecordMapper implements RecordMapper<GtpGroup>
{
    /** Serial version */
    private static final long serialVersionUID = 1769223168621L;

    /** The class log. */
    private static Log log
            = LogFactory.getLog(BaseGtpGroupRecordMapper.class);
            
    private static final String GROUP_ID_EXPRESSION = BaseGtpGroupPeer.GROUP_ID.getSqlExpression();
    private static final String GROUP_NAME_EXPRESSION = BaseGtpGroupPeer.GROUP_NAME.getSqlExpression();

    private boolean useMappingStrategy = true;

    private MappingStrategy<GtpGroup> strategy;
    
    public void initStrategy()
    {
        this.strategy = new MappingStrategy<GtpGroup>();
    }
     ...
     *         than the columns in the GTP_GROUP table.
     */
    public GtpGroup processRow(
                ResultSet resultSet,
                int offset,
                Criteria criteria)
            throws TorqueException
    {
        ...
        try 
        {
            gtpGroup.setLoading(true);
            if (criteria == null)
            {
                gtpGroup.setEntityId(
                        getEntityId(resultSet, offset + 1));
                gtpGroup.setEntityName(
            ...
            else
            {
                ...
                for (Column column : columnsWithoutOffset)
                {
                            ...
                            column.getSqlExpression()))
                    {
                        ...
                        {
                            gtpGroup.setEntityId( getEntityId(resultSet, totalOffset));
                        }
                        columnsMapped.add( GROUP_ID_EXPRESSION );
                    }
                    else if (!columnsMapped.contains(GROUP_NAME_EXPRESSION ) && GROUP_NAME_EXPRESSION.equals(
                            column.getSqlExpression()))
                    {
                        if (useMappingStrategy) 
                        {
                        ...
                        {
                            gtpGroup.setEntityName( getEntityName(resultSet, totalOffset));
                        }
                ...
                if (useMappingStrategy)
                {
                  ...
                  this.strategy.execute(resultSet, gtpGroup);
                }
            }
            gtpGroup.setNew(false);
            gtpGroup.setModified(false);
        }
        finally
        {
            gtpGroup.setLoading(false);
        }
        return gtpGroup;
    }
     ...
     * the field entityId in GtpGroup.
     *
     ...
     * Reads the value of the <code>columnIndex</code>'th column
     * in the <code>resultSet</code> so that it can be used to set
     * the field entityName in GtpGroup.
     *
     * @param resultSet the ResultSet to read from, not null.
     * @param columnIndex the index in the resultSet which should be read.
     *

...
public class SecureAction extends VelocitySecureAction {
	...
	protected boolean isAuthorized(PipelineData pipelineData) throws Exception {
		...
		if (acl == null) {
			...
			// Check for turbineadmin role (Flux default)
			isAuthorized = true;
		} else if (acl.hasRole("admin") || acl.hasRole("ADMIN") || acl.hasRole("Admin")) {
			// Also accept ADMIN role variations for compatibility
			isAuthorized = true;
		} else {
			data.setScreenTemplate((String) templateHomepage.getProperty(""));

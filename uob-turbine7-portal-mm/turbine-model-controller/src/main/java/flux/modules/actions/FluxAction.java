package com.uob.flux.modules.actions;
...
public class FluxAction extends VelocitySecureAction {
	...
	protected boolean isAuthorized(PipelineData pipelineData) throws Exception {
		...
		// Username fallback (same as FluxScreen and Menu.vm)
		...
		try {
			...
			if (user != null && user.getName() != null) {
				...
				if (username.contains("admin")) {
					...
					log.info("FluxAction.isAuthorized() - Username contains 'admin': " + user.getName());
				}
			}
		} catch (Exception e) {
			log.warn("FluxAction.isAuthorized() - Error checking username: " + e.getMessage());
		}
		...
		if (hasRole || usernameHasAdmin) {
			...
			String msg;
			try {
				msg = localizationService.getString(localizationService.getDefaultBundleName(),

...
import com.example.util.UserWrapperUtil;
...
public class LoginUserIntake extends org.apache.turbine.modules.actions.LoginUser {
	...
	public void doPerform(PipelineData pipelineData) throws FulcrumSecurityException 
	{
		...
		try 
		{
			...
			//data.setUser(user);
			User rawUser = security.getAuthenticatedUser(username, password);
			data.setUser(UserWrapperUtil.wrapUser(rawUser, true));

			// Mark the user as being logged in.
			rawUser.setHasLoggedIn(Boolean.TRUE);

			// Set the last_login date in the database.
			rawUser.updateLastLogin();
...
//			data.setUser(anonymousUser);
			...
			User anon = security.getAnonymousUser();
			data.setUser(UserWrapperUtil.wrapUser(anon, false));
